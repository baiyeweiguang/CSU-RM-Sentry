...
template <typename T> inline T getRollFromQuat(T w, T x, T y, T z) {
  return std::atan2(2 * (w * x + y * z), 1 - 2 * (x * x + y * y));
}

template <typename T> inline T getPitchFromQuat(T w, T x, T y, T z) {
  return std::asin(2 * (w * y - x * z));
}

template <typename T> inline T getYawFromQuat(T w, T x, T y, T z) {
  return std::atan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z));
}

template <typename EigenVector>
EigenVector interpolateVector(const EigenVector &vStart,
                              const EigenVector &vEnd, const Time &timeStart,
...
EigenQuaternion
interpolateQuaternion(const EigenQuaternion &qStart,
                      const EigenQuaternion &qEnd, const Time &timeStart,
                      const Time &timeEnd, const Time &queryTime) {
  const double duration = toSeconds(timeEnd - timeStart);
  const double factor = toSeconds(queryTime - timeStart) / duration;
  const Eigen::Quaterniond interp =
      Eigen::Quaterniond(qStart).slerp(factor, Eigen::Quaterniond(qEnd));
  return interp;
}
