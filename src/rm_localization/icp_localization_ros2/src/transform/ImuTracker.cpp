...
ImuTracker::ImuTracker()
    : currentOrientation_(Eigen::Quaterniond::Identity()),
      imuGravityTimeConstant_(0.01), currentGravityVector_(gravityVector),
      integratedPosition_(Eigen::Vector3d::Zero()),
      integratedLinearVelocity_(Eigen::Vector3d::Zero()),
      integratedRPY_(Eigen::Vector3d::Zero()), tPrev_(fromUniversal(0)) {
...
Eigen::Quaterniond ImuTracker::getOrientationChange(const Time &start,
  ...
  //  std::cout << "d RPY from tracker: " << dRPY.transpose() * 180.0/M_PI <<
  //  std::endl;
  return fromRPY(dRPY.x(), dRPY.y(), dRPY.z()).normalized();
}
Eigen::Vector3d
ImuTracker::getLienarPositionChange(const Time &start, const Time &end,
...
void ImuTracker::integrateImuReading(const Time &t,
  ...
  integratedLinearVelocity_ +=
      dt * removeGravity(reading.acceleration(), currentOrientation_);
...
Rigid3d ImuTracker::getLatestOdometry() const {
  ...
  return Rigid3d(
      integratedPosition_,
      fromRPY(integratedRPY_.x(), integratedRPY_.y(), integratedRPY_.z()));
}
...
void ImuTracker::fillIntegrationBuffer(const Time &start,
  ...
  const auto startTime =
      start < imuBuffer_.earliest_time() ? imuBuffer_.earliest_time() : start;
  const auto endTime =
  ...
  //    reading.imu_ =
  //    ImuReadingd(Eigen::Vector3d::Ones(),Eigen::Vector3d::Ones());
...
Eigen::Vector3d
ImuTracker::removeGravity(const Eigen::Vector3d &acc,
...

{
  ...
  for (int i = 1; i < integrationBuffer_.size(); ++i) {
    const double dt = toSeconds(integrationBuffer_.at(i).time_ -
...
void ImuTracker::updateOrientationFromLinearAcceleration() {
  ...
  const double dt = isSingleReading
                        ? std::numeric_limits<double>::infinity()
                        : toSeconds(imuReading.time_ -
  ...
  //  imuReading.imu_.acceleration().transpose() << "\n"; std::cout << "Gravity
  //  vector: " << currentGravityVector_.transpose() << "\n"; std::cout <<
  //  "Orientation: " << currentOrientation_.coeffs().transpose() << std::endl;
  //  std::cout << "current RPY: " << toRPY(currentOrientation_).transpose() *
  //  kRadToDeg << std::endl; std::cout << "\n";
}
