...
TwistIntegrationBuffer::TwistIntegrationBuffer() { setSizeLimit(kBuffSize); }

void TwistIntegrationBuffer::push(const Time &time, const Twist3d &twist) {
  // this relies that they will be pushed in order!!!
  if (!twists_.empty()) {
    if (time < earliest_time()) {
      std::cerr << "you are trying to push something earlier than the earliest "
                   "measurement, this should not happen \n";
    ...
    if (time < latest_time()) {
      std::cerr << "you are trying to push something out of order, this should "
                   "not happen \n";
      std::cerr << "ingnoring the mesurement \n";
      std::cerr << "Time: " << time << std::endl;
...
void TwistIntegrationBuffer::clear() { twists_.clear(); }
...
Twist3d TwistIntegrationBuffer::lookup(const Time &time) const {
  ...
  const auto end =
      std::lower_bound(twists_.begin(), twists_.end(), time,
                       [](const TimestampedTwist &twist, const Time time) {
...
Rigid3d TwistIntegrationBuffer::integrate(const Time &startTime,
                                          const Time &endTime) {
  if (empty()) {
  ...
  const auto lb =
      std::lower_bound(twists_.begin(), twists_.end(), startTime,
                       [](const TimestampedTwist &twist, const Time &time) {
                         return twist.time_ < time;
                       });
  const auto ub =
      std::upper_bound(twists_.begin(), twists_.end(), endTime,
...
bool TwistIntegrationBuffer::empty() const { return twists_.empty(); }

size_t TwistIntegrationBuffer::size_limit() const { return bufferSizeLimit_; }

size_t TwistIntegrationBuffer::size() const { return twists_.size(); }
